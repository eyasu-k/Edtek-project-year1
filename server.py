...
def receive_file(client: socket.socket, file_name: str, file_size: str, *_)-> None:# this function handles upload requests from the client.
    ...
    client.sendall(const.ACK.encode()) #sending an acknowledgement message to the client so that it can send the file contents
    debug("Sent an acknowledgement message to the client.")
    ...
    file_content = client.recv(int(file_size))#the download buffer size is adjusted to the file size.
